...
![Figure 1: Confusion Matrix — Log Level vs. Actual Error Semantics](figures/figure_1_confusion_matrix.png)

*Figure 1: Confusion matrix comparing log level field (ERROR vs. non-ERROR) against actual error semantics in a stratified sample of 500 HDFS log entries. 23% of error-indicating events were logged at non-ERROR levels, and 8% of ERROR-level entries were benign operational messages.*
...
![Figure 2: Error Event Concentration — BGL Dataset](figures/figure_2_error_concentration.png)

*Figure 2: Error event concentration across 4,800 one-minute time windows in the BGL dataset. Left: heavy-tailed distribution of errors per window (log scale). Right: cumulative concentration curve showing that the top 1% of windows contain ~18% of all error events.*
...
![Figure 3: Trace Reconstructability — Naive vs. Context-Preserving](figures/figure_3_trace_reconstructability.png)

*Figure 3: Trace reconstructability comparison on 200 sampled error traces at 200× compression ratio. Naive aggregation (counts/averages only) reconstructs 60.5% of traces; context-preserving aggregation (with sample trace IDs and error messages) reconstructs 95.5%.*
...
![Figure 4: Anomaly Scatter Plot — Error Rate vs. Latency](figures/figure_4_anomaly_scatter.png)

*Figure 4: Anomaly detection results on 500 Gold-layer service edges using KMeans (k=3, silhouette score=0.53). 5% of edges flagged as anomalous, classified by root cause: high error rate, high latency, combined, and traffic spikes.*
...
![Figure 5: Query Performance — Gold vs. Silver Layer](figures/figure_5_query_latency.png)

*Figure 5: Left: query execution time comparison showing Gold layer is 50–75× faster than Silver for equivalent aggregation queries. Right: data reduction ratios across LogHub datasets (134–201× reduction from Silver to Gold).*
...
![Figure 6: Streaming Anomaly Detection Architecture](figures/figure_6_streaming_architecture.png)

*Figure 6: Streaming anomaly detection architecture using Spark Structured Streaming. Log sources are ingested as micro-batches, aggregated via tumbling (1-min) and sliding (2-min) windows with 2-minute watermarks, then evaluated against threshold-based anomaly rules. Results flow to Delta Lake sinks for persistence and a memory sink for real-time dashboarding.*
...
> **Appendix**: All figures are generated by `scripts/generate_paper1_figures.py` and saved to `docs/figures/`. Source data and reproducibility instructions are available in the project repository.
