...
# Parse timestamp - use to_timestamp without format to auto-detect
...
# DBTITLE 1,Cell 24
# Stop previous queries first (only if still active)
if metrics_query.isActive:
    metrics_query.stop()
    print("⏸️  Stopped metrics query")
else:
    print("ℹ️  Metrics query already stopped")

if anomalies_query.isActive:
    anomalies_query.stop()
    print("⏸️  Stopped anomalies query")
else:
    print("ℹ️  Anomalies query already stopped")
...
# Stop query (only if still active)
if sliding_query.isActive:
    sliding_query.stop()
    print("⏸️  Stopped sliding window query")
