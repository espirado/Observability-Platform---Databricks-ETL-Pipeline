...
nosql_table = (nosql_edges
    .select(
        ...
        F.struct(
            F.col("request_count"),
            F.col("error_count"),
            F.col("avg_latency_ms").alias("avg_latency"),
            F.col("p50_latency_ms").alias("p50_latency"),
            F.col("p95_latency_ms").alias("p95_latency"),
            F.col("p99_latency_ms").alias("p99_latency")
        ).alias("metrics"),
        
        # cf:quality
        F.struct(
            F.col("error_rate"),
            (F.col("request_count") - F.col("error_count")).alias("success_count"),
            ((F.col("request_count") - F.col("error_count")) / F.col("request_count")).alias("success_rate")
        ).alias("quality"),
