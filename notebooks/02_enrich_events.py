...
if initial_count == 0:
    print("⚠️  No Bronze records found. Make sure notebook 01 has run successfully.")
...
flattened_df = (bronze_df
    # Service name (already exists at top level)
    .withColumn("service", F.coalesce(
        F.col("service"),
        F.lit("unknown")
    ))
    
    # HTTP fields (already flattened in Bronze)
    ...
    # Trace context (already at top level)
    .withColumn("trace_id", F.coalesce(F.col("trace_id"), F.lit(None)))
    .withColumn("span_id", F.lit(None).cast("string"))  # Not in Bronze
    .withColumn("parent_span_id", F.lit(None).cast("string"))  # Not in Bronze
    
    # Kubernetes metadata (already flattened)
    ...
    .withColumn("k8s_node", F.lit(None).cast("string"))  # Not in Bronze
    
    # Business context (not in Bronze - set to null)
    ...
    # Parse timestamp to proper format (handle microseconds with SSSSSS)
...
service_flow_df = (flattened_df
    ...
    .withColumn("endpoint",
        F.coalesce(F.col("http_path"), F.lit("/unknown")))
)
