...
latency_data = gold_pd.groupby("target_service")[["p50_latency", "p95_latency", "p99_latency"]].mean().reset_index()

fig3 = go.Figure()
for pct, color in [("p50_latency", "#2ca02c"), ("p95_latency", "#ff7f0e"), ("p99_latency", "#d62728")]:
...
fig5 = px.scatter(
    anomaly_pd, x="error_rate", y="avg_latency",
    ...
    labels={"error_rate": "Error Rate", "avg_latency": "Avg Latency (ms)"},
)
...
Average anomaly score: {anomaly_stats['anomaly_score'].mean():.3f if len(anomaly_stats) > 0 else 0}

Top anomalies (service, error_rate, latency, score):
{anomaly_stats.nlargest(10, 'anomaly_score')[['source_service','target_service','error_rate','avg_latency','anomaly_score']].to_string(index=False) if len(anomaly_stats) > 0 else 'None'}
...
chart_data = {
    ...
    "Latency Percentiles": f"P99 latency ranges from {latency_data['p99_latency'].min():.0f}ms to {latency_data['p99_latency'].max():.0f}ms across services.",
    ...
    "Anomaly Landscape": f"{len(anomaly_stats)} anomalies detected. Highest anomaly score: {anomaly_stats['anomaly_score'].max():.2f if len(anomaly_stats) > 0 else 0}.",
...
avg_latency      = gold_pd["avg_latency"].mean()
p95_lat          = gold_pd["p95_latency"].mean()
p99_lat          = gold_pd["p99_latency"].mean()
...
Latency by service:
{gold_pd.groupby('target_service')[['avg_latency','p95_latency','p99_latency']].mean().round(1).to_string()}
"""
...
if interpretation is None:
    ...
    high_lat_services = gold_pd[gold_pd["p95_latency"] > p95_lat * 1.5]["target_service"].unique()
...
class DataCopilot:
    ...
    def __init__(self, gold_pdf, anomaly_pdf, silver_pdf=None):
        ...
        self.stats = {
            ...
            "avg_latency":      round(gold_pdf["avg_latency"].mean(), 1),
            "p95_latency":      round(gold_pdf["p95_latency"].mean(), 1),
            "p99_latency":      round(gold_pdf["p99_latency"].mean(), 1),
...
"""
    ...
    def _ask_rules(self, question):
        ...
        if any(w in q for w in ["error rate", "error", "errors", "failure"]):
            ...
            lat = self.gold.groupby("target_service")[["avg_latency", "p95_latency", "p99_latency"]].mean().round(1)
